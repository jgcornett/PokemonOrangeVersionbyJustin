...
const GRAVITY = 800.0  # Adjust gravity value as needed
const TILE_SIZE = 16
const GRID_SIZE = 16
...
var wander_time = 0.0
var char_velocity = Vector2.ZERO  # Track character's velocity
...
func _physics_process(delta):
	# Apply gravity
	char_velocity.y += GRAVITY * delta

	if not is_on_floor():
		handle_movement()  # Handle movement when in the air
	else:
	...
	char_velocity.x = move_direction.x * SPEED

	move_and_slide()  # Slide along the ground
...
func _on_collision_shape_2d_body_entered(body):
	...
	# Show the beware message if the colliding body is of type CharacterBody2D
	if body is CharacterBody2D:
		$gomba/Label.visible = true
	else:
		$gomba/Label.visible = false
