...
# Mewtwo stats
var mewtwo_attack = 10
var mewtwo_defense = 28
var mewtwo_special_attack = 45
var mewtwo_special_defense = 35
var mewtwo_level = 10
...
func _ready():
	...
	$background/mewtwosprite1.visible = true
	$Label2.visible = false
	$Panel/exit.visible = false
	$Panel/Button2.visible = false
	

	# Display the message indicating Mewtwo's appearance
	$Label.text = "An evil mewtwo appeared\nSelect your move!"
...
func _on_nuzzle_pressed():
	if is_player_turn:
		...
		var damage = calculate_special_damage(pikachu_special_attack, mewtwo_special_defense, 40, pikachu_level, 1.0) * 0.7
		
		# Decrease enemy's HP
		enemy_hp -= damage
		
		# Lower Mewtwo's special defense
		mewtwo_special_defense -= int(mewtwo_special_defense * 0.1)
		
		# Display the message about Mewtwo's special defense being lowered
		_on_timer_2_timeout()
		
		if enemy_hp <= 0:
			enemy_hp = 0
			message += "\nMewtwo fainted!"
			$background/mewtwosprite1.visible = false
			# Set battle won flag
			battle_won = true
		...
		# Update UI and flags
		$Label.text = message
		$mewtwohp.value = enemy_hp
		
		# Check if player lost the battle
		if !battle_won:
			# Start delay before Mewtwo's turn
			delay_timer.start()
		else:
...
func _on_thunder_shock_pressed():
	if is_player_turn:
		...
		if randf() < 0.1:  # 10% chance
			damage = calculate_special_damage(mewtwo_special_attack, mewtwo_special_defense, 40, pikachu_level, 1.0) * 1.1  # Increase damage by 10% for critical hit
			message = "Critical hit!\n" + message
			critical_hit = true
		else:
			# Calculate normal special damage
			var type_effectiveness = 1.0
			if thunder_shock_type == "electric" and mewtwo_type == "water":
				type_effectiveness = 1.3
			damage = calculate_special_damage(mewtwo_special_attack, mewtwo_special_defense, 40, pikachu_level, type_effectiveness)
...
# AI function for Mewtwo's attack
func mewtwo_ai_attack():
	if enemy_action_complete or battle_won:
		print("Mewtwo's action is already complete or the battle is won. Skipping turn.")
		return
	...
	var move_index = randi() % 3  # Assuming Mewtwo has 3 moves
	match move_index:
		0:
			# Shadow Ball move
			$Label.text = "Mewtwo used Shadow Ball!"
			$Label2.visible = false
			shadow_ball_move()
		1:
			# Shadow Punch move
			$Label.text = "Mewtwo used Trump Flare!"
			$Label2.visible = false
			trump_flare_move()
		2:
			# Calm Mind move
			$Label.text = "Mewtwo used Calm Mind!\nHis special attack and defense rose!"
			$Label2.visible = false
			calm_mind_move()
		3:
			$Label.text = "Mewtwo used Psychic!"
			$Label2.visible = false
			psychic_move()
			
func psychic_move():
	var damage = calculate_special_damage(mewtwo_special_attack, pikachu_special_defense, 40, mewtwo_level, 1.0)  # Assuming type effectiveness is 1.0
	var message = "Mewtwo used Psychic!"
	var miss_chance = 0.5  # 50% chance to miss

	if randf() < miss_chance:
		message = "Mewtwo's Psychic missed!"
	else:
		player_hp -= damage
		if player_hp <= 0:
			player_hp = 0
...
func shadow_ball_move():
	var damage = calculate_special_damage(mewtwo_special_attack, pikachu_special_defense, 40, mewtwo_level, 1.0)  # Assuming type effectiveness is 1.0
	var message = "Mewtwo used Shadow Ball!"
	var miss_chance = 0.4  # 40% chance to miss

	if randf() < miss_chance:
		message = "Mewtwo's Shadow Ball missed!"
	else:
		player_hp -= damage
		if player_hp <= 0:
			player_hp = 0
...
# Shadow Punch move
# Trump Flare move
func trump_flare_move():
	var damage = calculate_physical_damage(mewtwo_attack, pikachu_defense, 40, mewtwo_level) * 0.8  # Physical damage halved
	var message = "Mewtwo used Trump Flare!"
	player_hp -= damage
	if player_hp <= 0:
		player_hp = 0
		message += "\nPikachu fainted!"
	...
	enemy_action_complete = true
	$Label.text = message
	$Label2.text = "Mewtwo's special attack rose!"
	# Increase Mewtwo's special attack
	mewtwo_special_attack += 10


# Calm Mind move
func calm_mind_move():
	$Label2.text = "Both special stats were raised!"
	# Increase Mewtwo's special attack and special defense by 10%
	mewtwo_special_attack += 10
	mewtwo_special_defense += 10
	enemy_action_complete = true
...
func _on_exit_pressed():
	get_tree().change_scene_to_file("res://theending.tscn")
...
func _on_timer_2_timeout():
	$Label2.text = "Mewtwo's special defense was lowered!"
